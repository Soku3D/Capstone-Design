...
namespace soku {
	...
	MeshData GeometryGenerator::MakeCylinder(int x, int y, float height, int radius)
	{
		...
		for (int height = 0; height <= y; height++)
		{
			for (int width = 0; width <= x; width++)
			{
				...
				p = Vector3::Transform(p, DirectX::SimpleMath::Matrix::CreateRotationY(delTheta * width));
				Vector2 uv = uvPoint + Vector2(width * uvDx, height * uvDy);
	...
	MeshData GeometryGenerator::MakeSphere(int x, int y, float radius, const std::string& texturePath)
	{
		...
		for (int height = 0; height <= y; height++)
		{
			for (int width = 0; width <= x; width++)
			{
				Vector3 p = Vector3::Transform(point, DirectX::SimpleMath::Matrix::CreateRotationZ(delThetaY * height));
				p = Vector3::Transform(p, DirectX::SimpleMath::Matrix::CreateRotationY(delThetaX * width));
				Vector2 uv = uvPoint + Vector2(width * uvDx, height * uvDy);
