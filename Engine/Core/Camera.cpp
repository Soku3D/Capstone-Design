...
Matrix Camera::GetCubeViewRow() const {
    return Matrix::CreateRotationY(-m_yaw) * Matrix::CreateRotationX(-m_pitch);
}
Matrix Camera::GetProjRow() const {
    return DirectX::XMMatrixPerspectiveFovLH(
        DirectX::XMConvertToRadians(m_projFovAngleY), m_aspect, m_nearZ,
        m_farZ);
}
Vector3 Camera::GetEyePos() const { return m_position; }
...
void Camera::MoveForward(float dt) { m_position += m_viewDir * dt * m_speed; }
void Camera::MoveRight(float dt) { m_position += m_rightDir * dt * m_speed; }
